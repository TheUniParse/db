...
-- initcap (initialize capital)
-- ⚠️ not available on all dbms's, pg support it!!
...
SELECT
	first_name,
	last_name,
	last_name,first_name || ' ' || last_name AS full_name
FROM employees;
...
-- Boolean expressios
-- return boolean based on salary compared to 50k
SELECT
...
-- wildcards are (%) represent any string
select
  first_name || ' ' || last_name as full_name,
  email
from employees
...
SELECT
	email,
	(email like '%.com%') AS dot_com,
	(email like '%.gov%') AS dot_gov
FROM employees;
...
FROM (
  ...
  ON s.city_id = l.city_id -- only rows in both tables
);
